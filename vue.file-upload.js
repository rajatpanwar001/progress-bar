var FileUploadComponent = Vue.extend({
  template:
    '<div class="{{ class }}"><label for="{{ name }}"><input type="file" name="{{ name }}" id="{{ id || name }}" accept="{{ accept }}" v-on:click="fileInputClick" v-on:change="fileInputChange" multiple="{{ multiple }}"><slot></slot></label><button type="button" v-on:click="fileUpload">{{ buttonText }}</button></div>',
  props: {
    class: String,
    name: {
      type: String,
  ...
  methods: {
    ...
    fileUpload: function () {
      if (this.myFiles.length > 0) {
        ...
        var arrayOfPromises = Array.prototype.slice.call(this.myFiles, 0).map(
          function (file) {
